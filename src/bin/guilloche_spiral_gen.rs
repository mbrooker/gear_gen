...
use gcode::{
    g0, g1, gcode_comment, preamble, trailer, xyz, xyzf, zf,
};
use std::f64;
...
fn generate_spiral(opt: &Opt, file: &mut dyn Write) -> Result<()> {
    ...
    g1(file, xyzf(start_x, 0., -opt.min_depth, opt.feed / 3.0))?;
    ...
    for turn in skip_turns..turns {
        ...
        for angle_step in 0..opt.steps_per_turn {
            ...
            let z = (opt.max_depth - opt.min_depth) * (1.0 + (angle * opt.rays as f64).cos()) + opt.min_depth;
            g1(file, xyzf(x, y, -z, opt.feed))?;
        } 
    }
    // Feed out
